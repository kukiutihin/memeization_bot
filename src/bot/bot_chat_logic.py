...
from bot.bot_functions import add_pic_from_pid, add_pic, remove_from_fav, send_images_any
...
async def waiting_tags_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database, ghosts: int):
    ...
    if len(tags) > 20:
        ...
        return
    a = search_memes_in_db(uid, ",".join(tags),db, 0.2, 10,10)
    await send_images_any(update, a)
